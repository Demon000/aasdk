...
class USBWrapperMock : public IUSBWrapper {
 public:
  ...
  MOCK_METHOD2(releaseInterface,
               int(const DeviceHandle& dev_handle, int interface_number));
  MOCK_METHOD1(getDevice, libusb_device*(const DeviceHandle& dev_handle));
  MOCK_METHOD2(claimInterface,
               int(const DeviceHandle& dev_handle, int interface_number));
  MOCK_METHOD2(openDeviceWithVidPid,
               DeviceHandle(uint16_t vendor_id, uint16_t product_id));
  MOCK_METHOD3(getConfigDescriptor,
               int(libusb_device* dev, uint8_t config_index,
                   ConfigDescriptorHandle& config_descriptor_handle));

  MOCK_METHOD8(fillBulkTransfer,
               void(libusb_transfer* transfer, const DeviceHandle& dev_handle,
                    unsigned char endpoint, unsigned char* buffer, int length,
                    libusb_transfer_cb_fn callback, void* user_data,
                    unsigned int timeout));

  MOCK_METHOD8(fillInterruptTransfer,
               void(libusb_transfer* transfer, const DeviceHandle& dev_handle,
                    unsigned char endpoint, unsigned char* buffer, int length,
                    libusb_transfer_cb_fn callback, void* user_data,
                    unsigned int timeout));

  MOCK_METHOD6(fillControlTransfer,
               void(libusb_transfer* transfer, const DeviceHandle& dev_handle,
  ...
  MOCK_METHOD6(fillControlSetup,
               void(unsigned char* buffer, uint8_t bmRequestType,
                    uint8_t bRequest, uint16_t wValue, uint16_t wIndex,
                    uint16_t wLength));
  MOCK_METHOD2(getDeviceDescriptor,
               int(libusb_device* dev, libusb_device_descriptor& desc));
  MOCK_METHOD0(handleEvents, void());
  MOCK_METHOD7(hotplugRegisterCallback,
               HotplugCallbackHandle(libusb_hotplug_event events,
                                     libusb_hotplug_flag flags, int vendor_id,
