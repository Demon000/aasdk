...
template <typename SourceResolveArgumentType = void,
          typename DestinationResolveArgumentType = void>
class PromiseLink
    : public std::enable_shared_from_this<PromiseLink<
          SourceResolveArgumentType, DestinationResolveArgumentType>> {
 public:
  typedef std::shared_ptr<
      PromiseLink<SourceResolveArgumentType, DestinationResolveArgumentType>>
      Pointer;
  typedef std::function<DestinationResolveArgumentType(
      SourceResolveArgumentType)>
      TransformFunctor;
  ...
  PromiseLink(typename Promise<DestinationResolveArgumentType>::Pointer promise,
      ...
      : promise_(std::move(promise)),
        transformFunctor_(
            std::make_shared<TransformFunctor>(std::move(transformFunctor))) {}

  static void forward(
      Promise<SourceResolveArgumentType>& source,
      typename Promise<DestinationResolveArgumentType>::Pointer destination,
      TransformFunctor transformFunctor =
          [](SourceResolveArgumentType&& argument) {
            return std::move(argument);
          }) {
    auto link = std::make_shared<
        PromiseLink<SourceResolveArgumentType, DestinationResolveArgumentType>>(
        std::forward<typename Promise<DestinationResolveArgumentType>::Pointer>(
            destination),
 ...
 private:
  using std::enable_shared_from_this<
      PromiseLink<SourceResolveArgumentType,
                  DestinationResolveArgumentType>>::shared_from_this;
  ...
  void reject(const error::Error& e) { promise_->reject(e); }

  typename Promise<SourceResolveArgumentType>::ResolveHandler
  getResolveHandler() {
    return std::bind(&PromiseLink::resolve, this->shared_from_this(),
                     std::placeholders::_1);
  }

  typename Promise<SourceResolveArgumentType>::RejectHandler
  getRejectHandler() {
    return std::bind(&PromiseLink::reject, this->shared_from_this(),
                     std::placeholders::_1);
  }
...
class PromiseLink<void, void>
    : public std::enable_shared_from_this<PromiseLink<void, void>> {
 public:
  ...
  static void forward(Promise<void>& source,
                      typename Promise<void>::Pointer destination) {
