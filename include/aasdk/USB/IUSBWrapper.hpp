...
class IUSBWrapper {
 public:
  ...
  virtual int releaseInterface(const DeviceHandle& dev_handle,
                               int interface_number) = 0;
  virtual libusb_device* getDevice(const DeviceHandle& dev_handle) = 0;
  virtual int claimInterface(const DeviceHandle& dev_handle,
                             int interface_number) = 0;
  virtual DeviceHandle openDeviceWithVidPid(uint16_t vendor_id,
                                            uint16_t product_id) = 0;
  virtual int getConfigDescriptor(
  ...
  virtual void fillControlTransfer(libusb_transfer* transfer,
                                   const DeviceHandle& dev_handle,
  ...
  virtual void fillControlSetup(unsigned char* buffer, uint8_t bmRequestType,
                                uint8_t bRequest, uint16_t wValue,
                                uint16_t wIndex, uint16_t wLength) = 0;
  virtual int getDeviceDescriptor(libusb_device* dev,
                                  libusb_device_descriptor& desc) = 0;
  virtual void handleEvents() = 0;
  virtual HotplugCallbackHandle hotplugRegisterCallback(
