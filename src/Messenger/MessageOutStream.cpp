...
void MessageOutStream::streamSplittedMessage() {
  try {
    ...
    auto size = remainingSize_ < cMaxFramePayloadSize ? remainingSize_
                                                      : cMaxFramePayloadSize;
    ...
    auto data(
        this->compoundFrame(frameType, common::DataConstBuffer(ptr, size)));
    ...
    if (frameType == FrameType::LAST) {
      ...
      transportPromise->then(
          [this, self = this->shared_from_this(), size]() mutable {
            offset_ += size;
            remainingSize_ -= size;
            this->streamSplittedMessage();
          },
          [this,
           self = this->shared_from_this()](const error::Error& e) mutable {
            this->reset();
            promise_->reject(e);
            promise_.reset();
          });
...
common::Data MessageOutStream::compoundFrame(
    FrameType frameType, const common::DataConstBuffer& payloadBuffer) {
  const FrameHeader frameHeader(message_->getChannelId(), frameType,
                                message_->getEncryptionType(),
                                message_->getType());
  common::Data data(frameHeader.getData());
  data.resize(data.size() + FrameSize::getSizeOf(frameType == FrameType::FIRST
  ...
  if (message_->getEncryptionType() == EncryptionType::ENCRYPTED) {
    ...
    data.insert(data.end(), payloadBuffer.cdata,
                payloadBuffer.cdata + payloadBuffer.size);
    payloadSize = payloadBuffer.size;
  }

  this->setFrameSize(data, frameType, payloadSize,
                     message_->getPayload().size());
  return data;
}

void MessageOutStream::setFrameSize(common::Data& data, FrameType frameType,
                                    size_t payloadSize, size_t totalSize) {
  ...
  memcpy(&data[FrameHeader::getSizeOf()], &frameSizeData[0],
         frameSizeData.size());
}
