...
FrameHeader::FrameHeader(const common::DataConstBuffer& buffer) {
  ...
  frameType_ = static_cast<FrameType>(buffer.cdata[1] &
                                      static_cast<uint32_t>(FrameType::BULK));
  encryptionType_ = static_cast<EncryptionType>(
      buffer.cdata[1] & static_cast<uint8_t>(EncryptionType::ENCRYPTED));
  messageType_ = static_cast<MessageType>(
      buffer.cdata[1] & static_cast<uint8_t>(MessageType::CONTROL));
}

FrameHeader::FrameHeader(ChannelId channelId, FrameType frameType,
...
common::Data FrameHeader::getData() const {
  ...
  data.push_back(static_cast<uint8_t>(encryptionType_) |
                 static_cast<uint8_t>(messageType_) |
