...
BOOST_FIXTURE_TEST_CASE(Messenger_Receive, MessengerUnitTest) {
  Messenger::Pointer themessenger(std::make_shared<Messenger>(
      ioService_, messageInStream_, messageOutStream_));
  themessenger->enqueueReceive(ChannelId::MEDIA_AUDIO,
                               std::move(receivePromise_));

  ReceivePromise::Pointer inStreamReceivePromise;
  EXPECT_CALL(messageInStreamMock_, startReceive(_))
      .WillOnce(SaveArg<0>(&inStreamReceivePromise));

  ioService_.run();
  ioService_.reset();

  Message::Pointer message(std::make_shared<Message>(ChannelId::MEDIA_AUDIO,
...
BOOST_FIXTURE_TEST_CASE(Messenger_DirectReceive, MessengerUnitTest) {
  ...
  ReceivePromise::Pointer inStreamReceivePromise;
  EXPECT_CALL(messageInStreamMock_, startReceive(_))
      .WillRepeatedly(SaveArg<0>(&inStreamReceivePromise));

  ioService_.run();
  ioService_.reset();

  Message::Pointer inputChannelMessage(std::make_shared<Message>(
      ChannelId::INPUT, EncryptionType::ENCRYPTED, MessageType::SPECIFIC));
  ...
  secondReceivePromise->then(
      ...
      std::bind(&ReceivePromiseHandlerMock::onReject,
                &receivePromiseHandlerMock_, std::placeholders::_1));
  themessenger->enqueueReceive(ChannelId::INPUT,
                               std::move(secondReceivePromise));
  ...
  Message::Pointer audioChannelMessage(std::make_shared<Message>(
      ChannelId::MEDIA_AUDIO, EncryptionType::ENCRYPTED,
      MessageType::SPECIFIC));
...
BOOST_FIXTURE_TEST_CASE(Messenger_Send, MessengerUnitTest) {
  ...
  EXPECT_CALL(messageOutStreamMock_, stream(message, _))
      .WillOnce(SaveArg<1>(&outStreamSendPromise));
...
BOOST_FIXTURE_TEST_CASE(Messenger_OnlyOneSendAtATime, MessengerUnitTest) {
  ...
  EXPECT_CALL(messageOutStreamMock_, stream(message, _))
      .Times(2)
      .WillRepeatedly(SaveArg<1>(&outStreamSendPromise));
