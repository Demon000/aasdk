...
size_t Cryptor::encrypt(common::Data& output,
  ...
  while (totalWrittenBytes < buffer.size) {
    const common::DataConstBuffer currentBuffer(buffer.cdata, buffer.size,
                                                totalWrittenBytes);
    const auto writeSize =
        sslWrapper_->sslWrite(ssl_, currentBuffer.cdata, currentBuffer.size);

    if (writeSize <= 0) {
      throw error::Error(error::ErrorCode::SSL_WRITE,
                         sslWrapper_->getError(ssl_, writeSize));
    }
...
size_t Cryptor::decrypt(common::Data& output,
                        const common::DataConstBuffer& buffer) {
  std::lock_guard<decltype(mutex_)> lock(mutex_);
  ...
  while (availableBytes > 0) {
    const auto& currentBuffer =
        common::DataBuffer(output, totalReadSize + beginOffset);
    auto readSize =
        sslWrapper_->sslRead(ssl_, currentBuffer.data, currentBuffer.size);

    if (readSize <= 0) {
      throw error::Error(error::ErrorCode::SSL_READ,
                         sslWrapper_->getError(ssl_, readSize));
    }
...
size_t Cryptor::read(common::Data& output) {
  ...
  while (totalReadSize < pendingSize) {
    const auto& currentBuffer =
        common::DataBuffer(output, totalReadSize + beginOffset);
    const auto readSize = sslWrapper_->bioRead(bIOs_.second, currentBuffer.data,
                                               currentBuffer.size);
...
void Cryptor::write(const common::DataConstBuffer& buffer) {
  ...
  while (totalWrittenBytes < buffer.size) {
    const common::DataConstBuffer currentBuffer(buffer.cdata, buffer.size,
                                                totalWrittenBytes);
    const auto writeSize = sslWrapper_->bioWrite(
        bIOs_.first, currentBuffer.cdata, currentBuffer.size);
...
const std::string Cryptor::cCertificate =
    "-----BEGIN CERTIFICATE-----\n\
...
const std::string Cryptor::cPrivateKey =
