...
AccessoryModeQueryChain::AccessoryModeQueryChain(
    IUSBWrapper& usbWrapper, boost::asio::io_service& ioService,
    ...
    if (promise_ != nullptr) {
      ...
      queryPromise->then(
          [this, self = this->shared_from_this()](
              IUSBEndpoint::Pointer usbEndpoint) mutable {
...
void AccessoryModeQueryChain::startQuery(
    AccessoryModeQueryType queryType, IUSBEndpoint::Pointer usbEndpoint,
...
void AccessoryModeQueryChain::manufacturerQueryHandler(
  ...
  this->startQuery(AccessoryModeQueryType::SEND_MODEL, std::move(usbEndpoint),
                   std::move(queryPromise));
}
...
void AccessoryModeQueryChain::descriptionQueryHandler(
  ...
  this->startQuery(AccessoryModeQueryType::SEND_VERSION, std::move(usbEndpoint),
                   std::move(queryPromise));
}
...
void AccessoryModeQueryChain::versionQueryHandler(
  ...
  this->startQuery(AccessoryModeQueryType::SEND_URI, std::move(usbEndpoint),
                   std::move(queryPromise));
}
...
void AccessoryModeQueryChain::uriQueryHandler(
  ...
  this->startQuery(AccessoryModeQueryType::SEND_SERIAL, std::move(usbEndpoint),
                   std::move(queryPromise));
}
...
void AccessoryModeQueryChain::serialQueryHandler(
  ...
  this->startQuery(AccessoryModeQueryType::START, std::move(usbEndpoint),
                   std::move(queryPromise));
}

void AccessoryModeQueryChain::startQueryHandler(
    IUSBEndpoint::Pointer usbEndpoint) {
  activeQuery_.reset();
