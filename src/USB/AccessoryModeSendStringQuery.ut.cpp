...
BOOST_FIXTURE_TEST_CASE(AccessoryModeSendStringQuery_SendString,
  ...
  EXPECT_CALL(
      usbWrapperMock_,
      fillControlSetup(
          NotNull(), LIBUSB_ENDPOINT_OUT | USB_TYPE_VENDOR, ACC_REQ_SEND_STRING,
          0, static_cast<uint16_t>(AccessoryModeSendStringType::MANUFACTURER),
          expectedQueryString.size() + 1));

  AccessoryModeSendStringQuery::Pointer query(
      std::make_shared<AccessoryModeSendStringQuery>(
          ioService_, usbWrapperMock_, usbEndpointMock_,
          AccessoryModeSendStringType::MANUFACTURER, expectedQueryString));
  ...
  const std::string actualQueryString(buffer.data + 8,
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeSendStringQuery_TransferError,
                        AccessoryModeSendStringQueryUnitTest) {
  common::DataBuffer buffer;
  IUSBEndpoint::Promise::Pointer usbEndpointPromise;
  EXPECT_CALL(*usbEndpointMock_, controlTransfer(_, _, _))
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeSendStringQuery_RejectWhenInProgress,
                        AccessoryModeSendStringQueryUnitTest) {
  common::DataBuffer buffer;
  IUSBEndpoint::Promise::Pointer usbEndpointPromise;
  EXPECT_CALL(*usbEndpointMock_, controlTransfer(_, _, _))
