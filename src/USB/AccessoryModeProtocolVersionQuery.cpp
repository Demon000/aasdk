...
AccessoryModeProtocolVersionQuery::AccessoryModeProtocolVersionQuery(
    boost::asio::io_service& ioService, IUSBWrapper& usbWrapper,
    IUSBEndpoint::Pointer usbEndpoint)
    : AccessoryModeQuery(ioService, std::move(usbEndpoint)) {
  data_.resize(8 + sizeof(ProtocolVersion));
  usbWrapper.fillControlSetup(&data_[0], LIBUSB_ENDPOINT_IN | USB_TYPE_VENDOR,
                              ACC_REQ_GET_PROTOCOL, 0, 0,
  ...
  strand_.dispatch([this, self = this->shared_from_this(),
    ...
    if (promise_ != nullptr) {
      ...
      usbEndpointPromise->then(
          [this,
           self = this->shared_from_this()](size_t bytesTransferred) mutable {
...
void AccessoryModeProtocolVersionQuery::protocolVersionHandler(
