...
AOAPDevice::AOAPDevice(IUSBWrapper& usbWrapper,
                       boost::asio::io_service& ioService, DeviceHandle handle,
...
IAOAPDevice::Pointer AOAPDevice::create(IUSBWrapper& usbWrapper,
  ...
  auto result =
      usbWrapper.claimInterface(handle, interfaceDescriptor->bInterfaceNumber);
...
ConfigDescriptorHandle AOAPDevice::getConfigDescriptor(IUSBWrapper& usbWrapper,
  ...
  auto result =
      usbWrapper.getConfigDescriptor(device, 0, configDescriptorHandle);
  if (result != 0) {
...
const libusb_interface* AOAPDevice::getInterface(
    const ConfigDescriptorHandle& configDescriptorHandle) {
...
const libusb_interface_descriptor* AOAPDevice::getInterfaceDescriptor(
