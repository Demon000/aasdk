...
void USBEndpoint::interruptTransfer(common::DataBuffer buffer, uint32_t timeout,
                                    Promise::Pointer promise) {
  if (endpointAddress_ == 0) {
    promise->reject(
        error::Error(error::ErrorCode::USB_INVALID_TRANSFER_METHOD));
    ...
    if (transfer == nullptr) {
      ...
      usbWrapper_.fillInterruptTransfer(transfer, handle_, endpointAddress_,
...
void USBEndpoint::bulkTransfer(common::DataBuffer buffer, uint32_t timeout,
                               Promise::Pointer promise) {
  if (endpointAddress_ == 0) {
    promise->reject(
        error::Error(error::ErrorCode::USB_INVALID_TRANSFER_METHOD));
    ...
    if (transfer == nullptr) {
      ...
      usbWrapper_.fillBulkTransfer(transfer, handle_, endpointAddress_,
                                   buffer.data, buffer.size,
                                   reinterpret_cast<libusb_transfer_cb_fn>(
                                       &USBEndpoint::transferHandler),
                                   this, timeout);
...
void USBEndpoint::transfer(libusb_transfer* transfer,
                           Promise::Pointer promise) {
  strand_.dispatch([this, self = this->shared_from_this(), transfer,
