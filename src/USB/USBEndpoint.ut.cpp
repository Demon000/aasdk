...
class USBEndpointUnitTest {
 protected:
  USBEndpointUnitTest()
    ...
    promise_->then(std::bind(&USBEndpointPromiseHandlerMock::onResolve,
                             &promiseHandlerMock_, std::placeholders::_1),
                   std::bind(&USBEndpointPromiseHandlerMock::onReject,
                             &promiseHandlerMock_, std::placeholders::_1));
  }
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_ControlTransferForNonControlEndpoint,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_, 0x01));

  EXPECT_CALL(
      promiseHandlerMock_,
      onReject(error::Error(error::ErrorCode::USB_INVALID_TRANSFER_METHOD)));
  EXPECT_CALL(promiseHandlerMock_, onResolve(_)).Times(0);
  usbEndpoint->controlTransfer(common::DataBuffer(data), 0,
                               std::move(promise_));

  ioService_.run();
}

BOOST_FIXTURE_TEST_CASE(USBEndpoint_BulkTransferForControlEndpoint,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_InterruptTransferForControlEndpoint,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_));
  ...
  EXPECT_CALL(
      ...
      onReject(error::Error(error::ErrorCode::USB_INVALID_TRANSFER_METHOD)));
  EXPECT_CALL(promiseHandlerMock_, onResolve(_)).Times(0);
  usbEndpoint->interruptTransfer(common::DataBuffer(data), 0,
                                 std::move(promise_));

  ioService_.run();
}

BOOST_FIXTURE_TEST_CASE(USBEndpoint_ControlTransferAllocationFailed,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_));
  ...
  EXPECT_CALL(
      promiseHandlerMock_,
      onReject(error::Error(error::ErrorCode::USB_TRANSFER_ALLOCATION)));
  EXPECT_CALL(promiseHandlerMock_, onResolve(_)).Times(0);
  usbEndpoint->controlTransfer(common::DataBuffer(data), 0,
                               std::move(promise_));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_BulkTransferAllocationFailed,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_, 0x01));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_InterruptTransferAllocationFailed,
                        USBEndpointUnitTest) {
  common::Data data(10, 0);
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_, 0x01));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_BulkTransfer, USBEndpointUnitTest) {
  ...
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_, endpointAddress));
  ...
  EXPECT_CALL(usbWrapperMock_,
              fillBulkTransfer(&transfer, _, endpointAddress, buffer.data,
                               buffer.size, _, _, _))
      .WillOnce(DoAll(SaveArg<5>(&transferCallback),
                      SaveArg<6>(&transfer.user_data)));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_MultipleBulkTransfers,
                        USBEndpointUnitTest) {
  const uint8_t endpointAddress = 0x55;
  USBEndpoint::Pointer usbEndpoint(std::make_shared<USBEndpoint>(
      usbWrapperMock_, ioService_, deviceHandle_, endpointAddress));
  ...
  libusb_transfer transfer;
  EXPECT_CALL(usbWrapperMock_, allocTransfer(0))
      .WillRepeatedly(Return(&transfer));
  ...
  for (size_t i = 0; i < attemptsCount; ++i) {
    ...
    promise->then(std::bind(&USBEndpointPromiseHandlerMock::onResolve,
                            &promiseHandlerMock_, std::placeholders::_1),
                  std::bind(&USBEndpointPromiseHandlerMock::onReject,
                            &promiseHandlerMock_, std::placeholders::_1));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_ControlTransfer, USBEndpointUnitTest) {
  ...
  common::DataBuffer buffer(data);
  EXPECT_CALL(usbWrapperMock_,
              fillControlTransfer(&transfer, _, buffer.data, _, _, _))
      .WillOnce(DoAll(SaveArg<3>(&transferCallback),
                      SaveArg<4>(&transfer.user_data)));
  EXPECT_CALL(usbWrapperMock_, submitTransfer(&transfer));
...
BOOST_FIXTURE_TEST_CASE(USBEndpoint_InterruptTransfer, USBEndpointUnitTest) {
  ...
  common::DataBuffer buffer(data);
  EXPECT_CALL(usbWrapperMock_,
              fillInterruptTransfer(&transfer, _, endpointAddress, buffer.data,
                                    buffer.size, _, _, _))
      .WillOnce(DoAll(SaveArg<5>(&transferCallback),
                      SaveArg<6>(&transfer.user_data)));
  EXPECT_CALL(usbWrapperMock_, submitTransfer(&transfer));
