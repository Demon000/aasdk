...
BOOST_FIXTURE_TEST_CASE(AccessoryModeStartQuery_Start,
                        AccessoryModeStartQueryUnitTest) {
  ...
  EXPECT_CALL(usbWrapperMock_,
              fillControlSetup(NotNull(), LIBUSB_ENDPOINT_OUT | USB_TYPE_VENDOR,
                               ACC_REQ_START, 0, 0, 0));

  AccessoryModeStartQuery::Pointer query(
      std::make_shared<AccessoryModeStartQuery>(ioService_, usbWrapperMock_,
                                                usbEndpointMock_));
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeStartQuery_TransferError,
                        AccessoryModeStartQueryUnitTest) {
  common::DataBuffer buffer;
  IUSBEndpoint::Promise::Pointer usbEndpointPromise;
  EXPECT_CALL(*usbEndpointMock_, controlTransfer(_, _, _))
...
BOOST_FIXTURE_TEST_CASE(AccessoryModeStartQuery_RejectWhenInProgress,
                        AccessoryModeStartQueryUnitTest) {
  common::DataBuffer buffer;
  IUSBEndpoint::Promise::Pointer usbEndpointPromise;
  EXPECT_CALL(*usbEndpointMock_, controlTransfer(_, _, _))
