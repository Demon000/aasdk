...
int USBWrapper::releaseInterface(const DeviceHandle& dev_handle,
...
int USBWrapper::claimInterface(const DeviceHandle& dev_handle,
                               int interface_number) {
  return libusb_claim_interface(dev_handle.get(), interface_number);
}

DeviceHandle USBWrapper::openDeviceWithVidPid(uint16_t vendor_id,
                                              uint16_t product_id) {
  auto raw_handle =
      libusb_open_device_with_vid_pid(usbContext_, vendor_id, product_id);
  return raw_handle != nullptr ? DeviceHandle(raw_handle, &libusb_close)
                               : DeviceHandle();
}

int USBWrapper::getConfigDescriptor(
  ...
  config_descriptor_handle =
      (result == 0 && raw_handle != nullptr)
          ? ConfigDescriptorHandle(raw_handle, &libusb_free_config_descriptor)
...
void USBWrapper::fillBulkTransfer(libusb_transfer* transfer,
  ...
  libusb_fill_bulk_transfer(transfer, dev_handle.get(), endpoint, buffer,
...
void USBWrapper::fillInterruptTransfer(libusb_transfer* transfer,
                                       ...
                                       void* user_data, unsigned int timeout) {
  libusb_fill_interrupt_transfer(transfer, dev_handle.get(), endpoint, buffer,
                                 length, callback, user_data, timeout);
}
...
void USBWrapper::fillControlTransfer(libusb_transfer* transfer,
                                     ...
                                     libusb_transfer_cb_fn callback,
                                     void* user_data, unsigned int timeout) {
  libusb_fill_control_transfer(transfer, dev_handle.get(), buffer, callback,
                               user_data, timeout);
}
...
int USBWrapper::open(libusb_device* dev, DeviceHandle& dev_handle) {
  ...
  dev_handle = (result == 0 && raw_handle != nullptr)
                   ? DeviceHandle(raw_handle, &libusb_close)
                   : DeviceHandle();
  return result;
}

void USBWrapper::fillControlSetup(unsigned char* buffer, uint8_t bmRequestType,
...
HotplugCallbackHandle USBWrapper::hotplugRegisterCallback(
  ...
  libusb_hotplug_register_callback(usbContext_, events, flags, vendor_id,
                                   product_id, dev_class, cb_fn, user_data,
                                   &raw_handle);
