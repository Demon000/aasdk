...
USBTransport::USBTransport(boost::asio::io_service& ioService,
...
void USBTransport::enqueueReceive(common::DataBuffer buffer) {
  ...
  aoapDevice_->getInEndpoint().bulkTransfer(buffer, cReceiveTimeoutMs,
...
void USBTransport::doSend(SendQueue::iterator queueElement,
  ...
  usbEndpointPromise->then(
      [this, self = this->shared_from_this(), queueElement,
       offset](size_t bytesTransferred) mutable {
        this->sendHandler(queueElement, offset, bytesTransferred);
      },
      [this, self = this->shared_from_this(),
       queueElement](const error::Error& e) mutable {
  ...
  aoapDevice_->getOutEndpoint().bulkTransfer(
      common::DataBuffer(queueElement->first, offset), cSendTimeoutMs,
      std::move(usbEndpointPromise));
}

void USBTransport::sendHandler(SendQueue::iterator queueElement,
                               common::Data::size_type offset,
