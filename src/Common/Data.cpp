...
DataBuffer::DataBuffer(Data::value_type* _data, Data::size_type _size,
...
DataBuffer::DataBuffer(void* _data, Data::size_type _size,
                       Data::size_type offset)
    : DataBuffer(reinterpret_cast<Data::value_type*>(_data), _size, offset) {}

DataBuffer::DataBuffer(Data& _data, Data::size_type offset)
    : DataBuffer(_data.empty() ? nullptr : &_data[0], _data.size(),
                 offset > _data.size() ? 0 : offset) {}
...
DataConstBuffer::DataConstBuffer() : cdata(nullptr), size(0) {}
...
DataConstBuffer::DataConstBuffer(const void* _data, Data::size_type _size,
                                 Data::size_type offset)
    : DataConstBuffer(reinterpret_cast<const Data::value_type*>(_data), _size,
                      offset) {}

DataConstBuffer::DataConstBuffer(const Data& _data, Data::size_type offset)
    : DataConstBuffer(_data.empty() ? nullptr : &_data[0], _data.size(),
                      offset > _data.size() ? 0 : offset) {}
...
std::string dump(const DataConstBuffer& buffer) {
  if (buffer.size == 0) {
    ...
    std::string hexDump =
        "[" + uint8_to_hex_string(buffer.cdata, buffer.size) + " ] ";
