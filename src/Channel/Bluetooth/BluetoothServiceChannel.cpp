...
void BluetoothServiceChannel::receive(
  ...
  receivePromise->then(
      ...
      std::bind(&IBluetoothServiceChannelEventHandler::onChannelError,
...
void BluetoothServiceChannel::sendBluetoothPairingResponse(
  ...
  message->insertPayload(
      messenger::MessageId(
          proto::ids::BluetoothChannelMessage::PAIRING_RESPONSE)
          .getData());
  message->insertPayload(response);
...
void BluetoothServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    case proto::ids::BluetoothChannelMessage::PAIRING_REQUEST:
      ...
      break;
    default:
      AASDK_LOG(error) << "[BluetoothServiceChannel] message not handled: "
                       << messageId.getId();
