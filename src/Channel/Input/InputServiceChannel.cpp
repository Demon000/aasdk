...
InputServiceChannel::InputServiceChannel(
    ...
    : ServiceChannel(strand, std::move(messenger),
                     messenger::ChannelId::INPUT) {}

void InputServiceChannel::receive(
    IInputServiceChannelEventHandler::Pointer eventHandler) {
  auto receivePromise = messenger::ReceivePromise::defer(strand_);
  receivePromise->then(
      std::bind(&InputServiceChannel::messageHandler, this->shared_from_this(),
...
void InputServiceChannel::sendInputEventIndication(
  ...
  message->insertPayload(
      messenger::MessageId(
          proto::ids::InputChannelMessage::INPUT_EVENT_INDICATION)
          .getData());
...
void InputServiceChannel::sendBindingResponse(
  ...
  auto message(std::make_shared<messenger::Message>(
      ...
      messenger::MessageType::SPECIFIC));
  message->insertPayload(
      messenger::MessageId(proto::ids::InputChannelMessage::BINDING_RESPONSE)
          .getData());
...
void InputServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    case proto::ids::ControlMessage::CHANNEL_OPEN_REQUEST:
      ...
      break;
    default:
      AASDK_LOG(error) << "[InputServiceChannel] message not handled: "
                       << messageId.getId();
...
void InputServiceChannel::handleBindingRequest(
...
void InputServiceChannel::handleChannelOpenRequest(
    const common::DataConstBuffer& payload,
    IInputServiceChannelEventHandler::Pointer eventHandler) {
