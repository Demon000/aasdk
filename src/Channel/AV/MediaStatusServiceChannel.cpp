...
MediaStatusServiceChannel::MediaStatusServiceChannel(
    ...
    : ServiceChannel(strand, std::move(messenger),
                     messenger::ChannelId::MEDIA_STATUS) {}

void MediaStatusServiceChannel::receive(
    IMediaStatusServiceChannelEventHandler::Pointer eventHandler) {
  auto receivePromise = messenger::ReceivePromise::defer(strand_);
  receivePromise->then(
      std::bind(&MediaStatusServiceChannel::messageHandler,
                this->shared_from_this(), std::placeholders::_1, eventHandler),
      std::bind(&IMediaStatusServiceChannelEventHandler::onChannelError,
...
void MediaStatusServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    default:
      AASDK_LOG(error) << "[MediaStatusServiceChannel] message not handled: "
...
void MediaStatusServiceChannel::handleMetadataUpdate(
...
void MediaStatusServiceChannel::handlePlaybackUpdate(
    const common::DataConstBuffer& payload,
    IMediaStatusServiceChannelEventHandler::Pointer eventHandler) {
