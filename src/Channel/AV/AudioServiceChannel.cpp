...
AudioServiceChannel::AudioServiceChannel(
    ...
    messenger::IMessenger::Pointer messenger, messenger::ChannelId channelId)
    : ServiceChannel(strand, std::move(messenger), channelId) {}

void AudioServiceChannel::receive(
    IAudioServiceChannelEventHandler::Pointer eventHandler) {
  auto receivePromise = messenger::ReceivePromise::defer(strand_);
  receivePromise->then(
      std::bind(&AudioServiceChannel::messageHandler, this->shared_from_this(),
                std::placeholders::_1, eventHandler),
      std::bind(&IAudioServiceChannelEventHandler::onChannelError, eventHandler,
                std::placeholders::_1));
...
void AudioServiceChannel::sendAVMediaAckIndication(
  ...
  message->insertPayload(
      messenger::MessageId(
          proto::ids::AVChannelMessage::AV_MEDIA_ACK_INDICATION)
          .getData());
...
void AudioServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    case proto::ids::AVChannelMessage::AV_MEDIA_WITH_TIMESTAMP_INDICATION:
      this->handleAVMediaWithTimestampIndication(payload,
    ...
    case proto::ids::ControlMessage::CHANNEL_OPEN_REQUEST:
      ...
      break;
    default:
      AASDK_LOG(error) << "[AudioServiceChannel] message not handled: "
                       << messageId.getId();
...
void AudioServiceChannel::handleAVChannelSetupRequest(
...
void AudioServiceChannel::handleStartIndication(
    const common::DataConstBuffer& payload,
    IAudioServiceChannelEventHandler::Pointer eventHandler) {
...
void AudioServiceChannel::handleStopIndication(
    const common::DataConstBuffer& payload,
    IAudioServiceChannelEventHandler::Pointer eventHandler) {
...
void AudioServiceChannel::handleChannelOpenRequest(
    const common::DataConstBuffer& payload,
    IAudioServiceChannelEventHandler::Pointer eventHandler) {
...
void AudioServiceChannel::handleAVMediaWithTimestampIndication(
    const common::DataConstBuffer& payload,
    IAudioServiceChannelEventHandler::Pointer eventHandler) {
