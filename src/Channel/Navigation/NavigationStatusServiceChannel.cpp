...
NavigationStatusServiceChannel::NavigationStatusServiceChannel(
    ...
    : ServiceChannel(strand, std::move(messenger),
                     messenger::ChannelId::NAVIGATION) {}

void NavigationStatusServiceChannel::receive(
    INavigationStatusServiceChannelEventHandler::Pointer eventHandler) {
  auto receivePromise = messenger::ReceivePromise::defer(strand_);
  receivePromise->then(
      std::bind(&NavigationStatusServiceChannel::messageHandler,
                this->shared_from_this(), std::placeholders::_1, eventHandler),
      std::bind(&INavigationStatusServiceChannelEventHandler::onChannelError,
...
void NavigationStatusServiceChannel::sendChannelOpenResponse(
...
void NavigationStatusServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    default:
      AASDK_LOG(error)
          << "[NavigationStatusServiceChannel] message not handled: "
          << messageId.getId() << " : " << dump(payload);
      this->receive(std::move(eventHandler));
      break;
  }
}

void NavigationStatusServiceChannel::handleChannelOpenRequest(
