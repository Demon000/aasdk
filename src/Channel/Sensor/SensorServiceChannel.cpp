...
SensorServiceChannel::SensorServiceChannel(
    ...
    : ServiceChannel(strand, std::move(messenger),
                     messenger::ChannelId::SENSOR) {}

void SensorServiceChannel::receive(
    ISensorServiceChannelEventHandler::Pointer eventHandler) {
  auto receivePromise = messenger::ReceivePromise::defer(strand_);
  receivePromise->then(
      std::bind(&SensorServiceChannel::messageHandler, this->shared_from_this(),
                std::placeholders::_1, eventHandler),
      std::bind(&ISensorServiceChannelEventHandler::onChannelError,
...
void SensorServiceChannel::sendChannelOpenResponse(
...
void SensorServiceChannel::messageHandler(
  ...
  switch (messageId.getId()) {
    ...
    case proto::ids::ControlMessage::CHANNEL_OPEN_REQUEST:
      ...
      break;
    default:
      AASDK_LOG(error) << "[SensorServiceChannel] message not handled: "
                       << messageId.getId();
...
void SensorServiceChannel::sendSensorEventIndication(
  ...
  message->insertPayload(
      messenger::MessageId(
          proto::ids::SensorChannelMessage::SENSOR_EVENT_INDICATION)
          .getData());
...
void SensorServiceChannel::sendSensorStartResponse(
  ...
  message->insertPayload(
      messenger::MessageId(
          proto::ids::SensorChannelMessage::SENSOR_START_RESPONSE)
          .getData());
  message->insertPayload(response);
...
void SensorServiceChannel::handleSensorStartRequest(
...
void SensorServiceChannel::handleChannelOpenRequest(
    const common::DataConstBuffer& payload,
    ISensorServiceChannelEventHandler::Pointer eventHandler) {
